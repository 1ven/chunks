...
const SlashedString_1 = require("../modules/SlashedString");
...
class CkRoute {
    ...
    path(req) {
        const path = RqMatched_1.isRouteRequest(req) ? (req.rest()) : (req.head().uri);
        return new SlashedString_1.SlashedString(path).read();
    }
}
